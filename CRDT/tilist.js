const { TiListNode } = require("./tilistNode.js");
...
class TiList {
  ...
  static fromSerialized(serializedContent) {
    const tiList = new TiList();
    serializedContent.forEach((node) => {
      const newNode = new TiListNode(
        node.id,
        node.value,
        node.next,
        node.isTombstone,
